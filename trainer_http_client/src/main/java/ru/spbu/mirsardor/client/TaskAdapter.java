package ru.spbu.mirsardor.client;
...
import ru.spbu.mirsardor.api.mapper.QuestionDtoMapper;
...
public class TaskAdapter implements QuestionRepository {
    ...
    public List<OpenQuestionCard> findAll() {
        List<OpenQuestionCard> tasks = Collections.emptyList();
        try {
    ...
    @Override
    public Optional<OpenQuestionCard> findById(Long code) {
        Optional<OpenQuestionCard> task = Optional.empty();
        try {
            String url = String.format(TASK_ROOT_URL_TEMPLATE, serverUrl);
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(url))
            ...
            if (validateResponse(response)) {
                ...
                List<OpenQuestionCard> tasks = mapper.mapToModel(dtos);
